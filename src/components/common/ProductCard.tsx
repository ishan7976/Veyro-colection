...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onQuickView }) => {
  ...
  return (
    <div
      className="group relative flex flex-col bg-white dark:bg-[#0A0A0A] border border-neutral-200 dark:border-white/10 hover:border-neutral-900 dark:hover:border-white/40 transition-all duration-300 cursor-pointer overflow-hidden shadow-xs hover:shadow-xl"
    ...
    >
      {/* Image Container */}
      <div className="relative aspect-[3/4] w-full overflow-hidden bg-neutral-100 dark:bg-zinc-950">
        <img
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5 items-start z-10">
          ...
          {product.gsm && (
            <span className="bg-neutral-900/80 dark:bg-black/80 text-white/90 dark:text-white/70 text-[9px] font-mono border border-neutral-700 dark:border-white/10 px-2 py-0.5">
              {product.gsm} GSM
            </span>
          )}
        </div>
        ...
        <button
          ...
          className={`absolute top-3 right-3 p-2 border backdrop-blur-md transition-all duration-200 z-10 ${
            inWishlist
        ...
        {/* Quick View & Quick Add Action Overlay */}
        <div className="absolute inset-x-3 bottom-3 flex flex-col gap-2 z-10 transition-all duration-300 transform translate-y-3 opacity-0 group-hover:translate-y-0 group-hover:opacity-100">
          {showSizeQuickPicker ? (
            <div
      ...
      {/* Info Content */}
      <div className="p-4 flex flex-col flex-1 justify-between bg-white dark:bg-[#0A0A0A]">
        <div>
          <div className="flex items-center justify-between text-[10px] text-neutral-500 dark:text-white/40 mb-1 font-mono uppercase tracking-widest">
          ...
          <h3 className="font-bold text-sm text-neutral-900 dark:text-white tracking-tight uppercase group-hover:text-neutral-600 dark:group-hover:text-white/80 transition-colors line-clamp-1">
            {product.name}
          </h3>
        </div>

        <div className="mt-3 pt-3 border-t border-neutral-100 dark:border-white/10 flex items-center justify-between">
          <div className="flex items-baseline gap-2 font-mono">
            <span className="font-bold text-sm text-neutral-900 dark:text-white">
              {formatPrice(product.price)}
            </span>
            {product.originalPrice && (
              <span className="text-xs text-neutral-400 dark:text-white/30 line-through">
          ...
          <div className="flex items-center gap-1">
            {product.colors.map((c) => (
              <span
                key={c.name}
                className="w-2.5 h-2.5 border border-neutral-300 dark:border-white/20 shadow-xs rounded-full"
